    ...
    while True:
        try:
            ...
            if cmd == 'step':
                ...
                # if random.random() < 1 - update_eps:  # 使用 simple agent 进行探索
                ...
                if done:  # 如果结束, 重新开一把
                    if info['result'] == constants.Result.Win:
                        win = 1
                    whole_obs = env.reset(train_idx=train_idx, goal=[1, 1, 0.2, 0.2, -0.5])  # 重新开一把
                ...
                whole_obs = env.reset(train_idx=train_idx, goal=[1, 1, 0.2, 0.2, -0.5])
                obs = featurize.featurize(whole_obs[train_idx])
                ...
                img_space = featurize.get_img_space()
                action_space = featurize.get_action_space()
                remote.send((img_space, action_space))
